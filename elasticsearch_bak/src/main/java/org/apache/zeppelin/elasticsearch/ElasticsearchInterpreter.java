...
import org.apache.zeppelin.interpreter.InterpreterPElasticsearchInterpretersult;
...
//      catch (JsonParseException e) {
  ...
  private InterpreterResult buildAggResponseMessage(Aggregations aggregations) {
    ...
    if (agg instanceof InternalSingleBucketAggregation) {
...
//      for ( InternalMultiBucketAggregation.InternalBucket bucket : multiBucketAgg.getBuckets()) {
//        buffer.append("\n")
...
//    }
  ...
  private String buildSearchHitsResponseMessage(SearchHit[] hits) {
    ...
    for (SearchHit hit : hits) {
      ...
      if (json == null) {
        hitFields.clear();
        for (DocumentField hitField : hit.getFields().values()) {
